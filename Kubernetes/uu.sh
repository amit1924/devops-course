...
show_download_info() {
    clear
    echo -e "${PURPLE}==============================${NC}"
    echo -e "${WHITE}        YDL Downloader        ${NC}"
    echo -e "${PURPLE}==============================${NC}"
...
custom_quality() {
    ...
    yt-dlp -f "bv*[height<=${height}]+ba/best" --merge-output-format mkv "$url"
}
...
playlist_download() {
    ...
    case "$playlist_option" in
        1)
            yt-dlp -f "bv*+ba/best" --yes-playlist "$url"
            ;;
        2)
            read -p "Start from video number: " start
            read -p "End at video number: " end
            yt-dlp -f "bv*+ba/best" --playlist-start $start --playlist-end $end "$url"
            ;;
        3)
            yt-dlp -f "bv*+ba/best" --no-playlist "$url"
            ;;
        *)
            yt-dlp -f "bv*+ba/best" --yes-playlist "$url"
            ;;
    ...
    while true; do
        ...
        case "$option" in
            ...
            1)
                ...
                simulate_progress
                yt-dlp -f "bv*+ba/best" --merge-output-format mkv "$url"
                ;;
            2)
                show_download_info "1080p Video" "$url"
                simulate_progress
                yt-dlp -f "bv*[height<=1080]+ba/best" --merge-output-format mkv "$url"
                ;;
            3)
                show_download_info "720p Video" "$url"
                simulate_progress
                yt-dlp -f "bv*[height<=720]+ba/best" "$url"
                ;;
            4)
                show_download_info "480p Video" "$url"
                simulate_progress
                yt-dlp -f "bv*[height<=480]+ba/best" "$url"
                ;;
            5)
